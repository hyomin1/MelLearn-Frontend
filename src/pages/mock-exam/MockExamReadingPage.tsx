...
export default function MockExamReadingPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const mockExam = useMockExamStore((state) => state.mockExam);
  const { readingQuiz } = mockExam as MockExam;
  const quizLen = readingQuiz.quizzes.length;
  ...
  const currentQuiz = readingQuiz.quizzes[index];
  const answers = mockExamProgress.reading.answers as number[];

  const [isLast, setIsLast] = useState(false);
  const categoryColor = getCategoryColor('reading');
  ...
  const onChangeAnswer = (answer: number) => {
    ...
    newArr[index] = answer;
    setMockExamProgress('reading', { completed: true, answers: newArr });
  };
  ...
  const handleComplete = () => {
    setMockExamProgress('reading', { completed: true, answers });
